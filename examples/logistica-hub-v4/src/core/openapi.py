...
class RouteRegistry:
    ...
    def get(self, path: str, summary: str = "", tags: list = None, description: str = "", params: list = None, sample_response: dict = None):
        def decorator(fn):
            self.routes["GET"][path] = fn
            self.endpoints.append({
                "id": path.replace("/", "_").strip("_"),
                "method": "GET",
                "path": path,
                "title": summary or path,
                "tag": (tags[0] if tags else "Geral"),
                "desc": description or summary or f"Endpoint HTTP GET para {path}",
                "auth": "Bearer Token / Sessão Ativa",
                "params": params or [],
                "body": None,
                "sample_response": sample_response or {"status": 200, "mensagem": "Sucesso"}
            })
            return fn
        return decorator

    def post(self, path: str, summary: str = "", tags: list = None, description: str = "", params: list = None, body: dict = None, sample_response: dict = None):
        def decorator(fn):
            ...
            self.endpoints.append({
                ...
                "title": summary or path,
                "tag": (tags[0] if tags else "Geral"),
                "desc": description or summary or f"Endpoint HTTP POST para {path}",
                "auth": "Bearer Token / API Key",
                "params": params or [],
                "body": body or {},
                "sample_response": sample_response or {"sucesso": True}
            })
            return fn
        return decorator
    ...
    def generate_openapi_json(self, title: str, version: str):
        ...
        return {
            "openapi": "3.1.0",
            "info": {"title": title, "version": version, "description": "API Reference Dinâmica Logística Suite v4.0"},
            "paths": paths_obj
        }
...
<body>
    <header>
        <div class="brand-group">
            <div class="brand-title">
                ...
                <span>Logística Hub Suite v4.0</span>
    ...
    <div class="studio-layout">
        ...
        <main class="doc-column" id="doc-main-area">
            <div class="doc-tag-badge" id="doc-tag">Frotas</div>
            ...
            <table class="params-table">
                ...
                <tbody id="params-table-body">
                    <tr><td colspan="3" style="color: var(--text-muted);">Nenhum parâmetro necessário.</td></tr>
                </tbody>
            </table>
