# -*- coding: utf-8 -*-
"""
Serviço de regras de negócio Full CRUD e publicação de eventos para o módulo 'erp'.
"""
...
class ErpService:
    ...
    def listar(self, apenas_ativos: bool = True, status: Optional[str] = None) -> List[Dict[str, Any]]:
        with self.db.get_connection() as conn:
            query = "SELECT * FROM mod_erp WHERE 1=1"
            params = []
            if apenas_ativos:
                query += " AND ativo = 1"
            if status:
    ...
    def obter_por_id(self, item_id: int) -> Optional[Dict[str, Any]]:
        with self.db.get_connection() as conn:
            row = conn.execute("SELECT * FROM mod_erp WHERE id = ?", (item_id,)).fetchone()
    ...
    def criar(self, titulo: str, dados: Optional[Dict[str, Any]] = None, descricao: str = "", status: str = "ativo") -> Dict[str, Any]:
        ...
        with self.db.get_connection() as conn:
            cur = conn.execute(
                """
                INSERT INTO mod_erp (titulo, descricao, dados_json, status, ativo)
                VALUES (?, ?, ?, ?, 1)
                """,
        ...
        if self.events:
            self.events.emit("erp_criado", payload)
    ...
    def atualizar(self, item_id: int, titulo: Optional[str] = None, dados: Optional[Dict[str, Any]] = None, descricao: Optional[str] = None, status: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.events:
            self.events.emit("erp_atualizado", payload)
    ...
    def deletar(self, item_id: int) -> Dict[str, Any]:
        with self.db.get_connection() as conn:
            ...
            if not row:
                return {"sucesso": False, "erro": "Item não encontrado"}
            conn.execute("DELETE FROM mod_erp WHERE id = ?", (item_id,))
            conn.commit()

        if self.events:
            self.events.emit("erp_deletado", {"id": item_id})
