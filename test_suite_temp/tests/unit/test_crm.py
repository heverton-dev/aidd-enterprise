# -*- coding: utf-8 -*-
"""
Suíte de testes unitários para o módulo 'crm'.
...
from modules.crm.models import init_schema
...
def test_env(tmp_path):
    ...
    events = EventBus()
    eventos_capturados = []
    events.on("crm_criado", lambda d: eventos_capturados.append(("criado", d)))
    events.on("crm_atualizado", lambda d: eventos_capturados.append(("atualizado", d)))
    events.on("crm_deletado", lambda d: eventos_capturados.append(("deletado", d)))

    service = CrmService(db, events)
    return {"service": service, "events": eventos_capturados, "db": db}


def test_fluxo_completo_crud_crm(test_env):
    service = test_env["service"]
    eventos = test_env["events"]

    # 1. CREATE
    res_cria = service.criar(
        titulo="Item Teste Unitário Crm",
    ...
    assert item["titulo"] == "Item Teste Unitário Crm"
    ...
    res_up = service.atualizar(
        item_id=novo_id,
        titulo="Item Teste Crm Atualizado",
        status="concluido"
    )
    ...
    assert item_mod["titulo"] == "Item Teste Crm Atualizado"
...
def test_validacao_titulo_obrigatorio_crm(test_env):
    service = test_env["service"]
