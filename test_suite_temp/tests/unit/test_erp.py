# -*- coding: utf-8 -*-
"""
Suíte de testes unitários para o módulo 'erp'.
...
from modules.erp.models import init_schema
...
def test_env(tmp_path):
    ...
    events = EventBus()
    eventos_capturados = []
    events.on("erp_criado", lambda d: eventos_capturados.append(("criado", d)))
    events.on("erp_atualizado", lambda d: eventos_capturados.append(("atualizado", d)))
    events.on("erp_deletado", lambda d: eventos_capturados.append(("deletado", d)))

    service = ErpService(db, events)
    return {"service": service, "events": eventos_capturados, "db": db}


def test_fluxo_completo_crud_erp(test_env):
    service = test_env["service"]
    eventos = test_env["events"]

    # 1. CREATE
    res_cria = service.criar(
        titulo="Item Teste Unitário Erp",
    ...
    assert item["titulo"] == "Item Teste Unitário Erp"
    ...
    res_up = service.atualizar(
        item_id=novo_id,
        titulo="Item Teste Erp Atualizado",
        status="concluido"
    )
    ...
    assert item_mod["titulo"] == "Item Teste Erp Atualizado"
...
def test_validacao_titulo_obrigatorio_erp(test_env):
    service = test_env["service"]
