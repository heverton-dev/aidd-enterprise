# -*- coding: utf-8 -*-
"""
Suíte de testes unitários para o módulo 'helpdesk'.
...
from modules.helpdesk.models import init_schema
...
def test_env(tmp_path):
    ...
    db_file = str(tmp_path / "test_helpdesk.db")
    ...
    events = EventBus()
    eventos_capturados = []
    events.on("helpdesk_criado", lambda d: eventos_capturados.append(("criado", d)))
    events.on("helpdesk_atualizado", lambda d: eventos_capturados.append(("atualizado", d)))
    events.on("helpdesk_deletado", lambda d: eventos_capturados.append(("deletado", d)))

    service = HelpdeskService(db, events)
    return {"service": service, "events": eventos_capturados, "db": db}


def test_fluxo_completo_crud_helpdesk(test_env):
    service = test_env["service"]
    eventos = test_env["events"]

    # 1. CREATE
    res_cria = service.criar(
        titulo="Item Teste Unitário Helpdesk",
    ...
    assert item["titulo"] == "Item Teste Unitário Helpdesk"
    ...
    res_up = service.atualizar(
        item_id=novo_id,
        titulo="Item Teste Helpdesk Atualizado",
        status="concluido"
    )
    ...
    assert item_mod["titulo"] == "Item Teste Helpdesk Atualizado"
...
def test_validacao_titulo_obrigatorio_helpdesk(test_env):
    service = test_env["service"]
